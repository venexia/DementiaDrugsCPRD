...
global project "$path/projects/DementiaDrugsCPRD_v3"
...
local c = 0
foreach x in "diagnosis_dem==1" "(diagnosis_dem==1 | diagnosis_dem==2)" "diagnosis_dem>0" "diagnosis_dem>1" {
	qui {
		local c = `c'+1
...
* Generate news search figure:

run "$dofiles/run_trendnews.do"
